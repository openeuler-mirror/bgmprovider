 ...
 */
public final class SDFRSACipher extends CipherSpi {
        ...
        try {
            switch (mode) {
                case MODE_SIGN:
                    ...
                    if (paddingCopy != null) {
                        result = SDFRSACore.rsa(paddingCopy, privateKey);
                    } else {
                ...
                case MODE_VERIFY:
                    ...
                    SDFRSACore.rsa(verifyBuffer, publicKey);
                    ...
                    break;
                case MODE_ENCRYPT:
                    // NoPadding mode, the front of the message is filled with 0
                    if (paddingType == PAD_NONE) {
                        paddingCopy = SDFRSACore.prefixZero(buffer, 0, bufOfs);
                    } else {
                        paddingCopy = padding.pad(buffer, 0, bufOfs);
                    }
                    if (paddingCopy != null) {
                        result = SDFRSACore.rsa(paddingCopy, publicKey);
                    } else {
                        throw new BadPaddingException
                ...
                case MODE_DECRYPT:
                    ...
                    paddingCopy = SDFRSACore.rsa(decryptBuffer, privateKey);
                    result = padding.unpad(paddingCopy);
...
//            return ConstructKeys.constructKey(encoded, algorithm, type);
            // todo return ConstructKeys.constructKey(encoded, algorithm, type);
            return null;
        } finally {
            if (encoded != null) {
                Arrays.fill(encoded, (byte) 0);
