...
public class SDFSM9KeyPairGenerator extends SDFKeyPairGeneratorCore {
    private SDFSM9ParameterSpec paramSpec = null;

    public SDFSM9KeyPairGenerator() {
        super(256);
    }
    ...
    protected void checkParameterSpec(AlgorithmParameterSpec params) throws InvalidAlgorithmParameterException {
        SDFSM9ParameterSpec spec;
        if (params instanceof SDFSM9ParameterSpec) {
            spec = (SDFSM9ParameterSpec) params;
        } else {
            throw new InvalidAlgorithmParameterException("SDFSM9ParameterSpec required for SM9 Enc");
        }
        paramSpec = spec;
    }
    ...
    protected byte[][] implGenerateKeyPair(SDFKEKInfoEntity kekInfo, int keySize) throws SDFException {
        return SDFSM9KeyPairGeneratorNative.nativeGenerateKeyPair(
                SDFDataKeyType.DATA_KEY_SM9_MASTER_ENC.getType(),
                kekInfo.getKekId(),
                kekInfo.getRegionId(),
                kekInfo.getCdpId(),
                kekInfo.getPin()
    ...
    public KeyPair generateKeyPair() {
        ...
        byte[] userPriKey = SDFSM9KeyPairGeneratorNative
                .nativeCreateUserPriKey(SDFDataKeyType.DATA_KEY_SM9_USER_ENC.getType(),
