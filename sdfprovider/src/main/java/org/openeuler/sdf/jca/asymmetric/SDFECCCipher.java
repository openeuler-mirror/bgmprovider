...
import org.openeuler.sdf.wrapper.SDFECCCipherNative;
 ...
 * SDF ECC Cipher
 */
public class SDFECCCipher extends CipherSpi {
    ...
    private byte[] encrypt(byte[] in) {
        ...
        try {
            ...
            cipherParams = SDFECCCipherNative.nativeECCEncrypt(pubKeyArr, in);
    ...
    private byte[] decrypt(byte[] in) {
        ...
        try {
            ...
            byte[][] eccCipherParams = SDFUtil.decodeECCCipher(outputMode, in, curveLength);
            result = SDFECCCipherNative.nativeECCDecrypt(priKeyArr, pinArr, eccCipherParams);
        } catch (Exception e) {
