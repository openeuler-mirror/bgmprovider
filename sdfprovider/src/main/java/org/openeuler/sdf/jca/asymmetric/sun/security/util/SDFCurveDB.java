...
public class SDFCurveDB {
    ...
    private static final Map<String, SDFNamedCurve> oidMap =
        new LinkedHashMap<String, SDFNamedCurve>();
    private static final Map<String, SDFNamedCurve> nameMap =
        new HashMap<String, SDFNamedCurve>();
    private static final Map<Integer, SDFNamedCurve> lengthMap =
        new HashMap<Integer, SDFNamedCurve>();

    private static Collection<? extends SDFNamedCurve> specCollection;

    public static final String SPLIT_PATTERN = ",|\\[|\\]";

    // Used by SunECEntries
    public static Collection<? extends SDFNamedCurve>getSupportedCurves() {
        return specCollection;
    }

    // Return a NamedCurve for the specified OID/name or null if unknown.
    static SDFNamedCurve lookup(String name) {
        SDFNamedCurve spec = oidMap.get(name);
        if (spec != null) {
            return spec;
        }
    ...
    static SDFNamedCurve lookup(int length) {
    ...
    static SDFNamedCurve lookup(ECParameterSpec params) {
        if ((params instanceof SDFNamedCurve) || (params == null)) {
            return (SDFNamedCurve)params;
        }
        ...
        for (SDFNamedCurve namedCurve : specCollection) {
    ...
    private static void add(String name, String soid, int type, String sfield,
        ...
        SDFNamedCurve params = new SDFNamedCurve(name, soid, curve, g, bi(n), h);
    ...
    static String[] getNamesByOID(String oid) {
        SDFNamedCurve nc = oidMap.get(oid);
        if (nc == null) {
            return new String[0];
        }
