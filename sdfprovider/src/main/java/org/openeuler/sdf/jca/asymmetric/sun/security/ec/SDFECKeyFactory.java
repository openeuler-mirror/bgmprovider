 ...
 */
public final class SDFECKeyFactory extends KeyFactorySpi {
    ...
    private static KeyFactory getInstance() {
        if (instance == null) {
            try {
                instance = KeyFactory.getInstance("EC", "SDFProvider");
    ...
    private static void checkKey(ECKey key) throws InvalidKeyException {
        ...
        if (key instanceof ECPublicKey) {
            if (key instanceof SDFECPublicKeyImpl) {
                return;
            }
    ...
    private PublicKey implTranslatePublicKey(PublicKey key)
        ...
        if (key instanceof ECPublicKey) {
            ...
            return new SDFECPublicKeyImpl(
                ecKey.getW(),
                ecKey.getParams()
            );
        } else if ("X.509".equals(key.getFormat())) {
            byte[] encoded = key.getEncoded();
            return new SDFECPublicKeyImpl(encoded);
        } else {
    ...
    private PrivateKey implTranslatePrivateKey(PrivateKey key)
        ...
        if (key instanceof ECPrivateKey) {
            ...
            return new SDFECPrivateKeyImpl(
                ecKey.getS(),
                ecKey.getParams()
            );
        } else if ("PKCS#8".equals(key.getFormat())) {
            return new SDFECPrivateKeyImpl(key.getEncoded());
        } else {
    ...
    private PublicKey implGeneratePublic(KeySpec keySpec)
        ...
        if (keySpec instanceof X509EncodedKeySpec) {
            ...
            return new SDFECPublicKeyImpl(x509Spec.getEncoded());
