...
public class SDFIvParameterSpec extends IvParameterSpec implements SDFCipherHeadSpec {
    // enable cipher head
    private boolean withCipherHead;
    private byte[] cipherHeadBytes;

    public SDFIvParameterSpec(byte[] iv) {
        super(iv);
    }

    public SDFIvParameterSpec(byte[] iv, boolean withCipherHead) {
        super(iv);
        initHead(withCipherHead);
    }

    public SDFIvParameterSpec(byte[] iv, int offset, int len) {
        super(iv, offset, len);
    }

    public SDFIvParameterSpec(byte[] iv, int offset, int len, boolean withCipherHead) {
        super(iv, offset, len);
        initHead(withCipherHead);
    }

    private void initHead(boolean withCipherHead) {
        if (withCipherHead) {
