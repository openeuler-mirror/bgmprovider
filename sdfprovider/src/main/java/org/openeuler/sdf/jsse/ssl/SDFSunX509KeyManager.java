...
import org.openeuler.sdf.jsse.commons.SDFGMConstants;
import org.openeuler.sdf.jsse.commons.SDFGMTlsUtil;
...
public class SDFSunX509KeyManager extends X509ExtendedKeyManager {
    ...
    SDFSunX509KeyManager(KeyStore ks, char[] password, byte[] pin)
        ...
        for (Enumeration<String> aliases = ks.aliases();
            ...
            PrivateKey privateKey;
            if (key instanceof SDFECPrivateKeyImpl) {
                privateKey = new SDFECPrivateKeyImpl(key.getEncoded(), pin);
            } else {
            ...
            X509Credentials cred = new X509Credentials(privateKey, (X509Certificate[])certs);
    ...
    private String[] getAliases(String keyType, Principal[] issuers) {
        ...
        for (Map.Entry<String,X509Credentials> entry :
            ...
            // Filter invalid EC or SM2 certificate.
            if (SDFGMTlsUtil.isInvalidECOrSM2Cert(keyType, certs[0].getSigAlgName())) {
                continue;
            }

            if (!SDFGMConstants.equalsAlgorithm(keyType, certs[0].getPublicKey().getAlgorithm())) {
                continue;
            }
            if (sigType != null) {
                if (certs.length > 1) {
