...
#include "org_openeuler_sdf_wrapper_SDFECCCipherNative.h"
#include "sdf_exception.h"
#include "sdf_util.h"
#include "sdf_log.h"
#include <stdlib.h>

JNIEXPORT jobject JNICALL Java_org_openeuler_sdf_wrapper_SDFECCCipherNative_nativeECCEncrypt(JNIEnv *env, jclass cls,
        jobjectArray pubKeyArr, jbyteArray pubDataArr) {
    unsigned int keyType = DATA_KEY_ECC;
    ...
    // encrypt
    if ((rv = CDM_AsymEncrypt(keyType, publicKey, pubKeyLen, data, dataLen, encData, &encDataLen)) != SDR_OK) {
...
JNIEXPORT jbyteArray JNICALL Java_org_openeuler_sdf_wrapper_SDFSM2CipherNative_nativeSM2Decrypt(JNIEnv *env, jclass cls,
        jbyteArray priKeyArr, jbyteArray pinArr, jobjectArray cipherParams, jboolean enableAcc) {
    unsigned int keType = DATA_KEY_ECC;
    void *keyHandle = NULL;
    unsigned char *encData = NULL;
    ...
    // keyHandle
    keyHandle = SDF_CreateSM2PriKeyHandle(env, priKeyArr, pinArr, enableAcc);
    ...
    // decrypt
    if (enableAcc) {
        if ((rv = CDM_AsymDecryptEx(keType, keyHandle, encData, encDataLen, 0, data, &dataLen, NULL, 0) != SDR_OK)) {
            throwSDFException(env, rv, "CDM_AsymDecryptEx");
            goto cleanup;
        }
    } else {
        if ((rv = CDM_AsymDecrypt(keType, keyHandle, encData, encDataLen, data, &dataLen) != SDR_OK)) {
