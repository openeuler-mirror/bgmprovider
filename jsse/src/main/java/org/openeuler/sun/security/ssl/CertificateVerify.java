...
final class CertificateVerify {
    ...
    static final SSLConsumer gmtlsHandshakeConsumer =
        new GMTlsCertificateVerifyConsumer();
    static final HandshakeProducer gmtlsHandshakeProducer =
        new GMTlsCertificateVerifyProducer();
    ...
    static final class GMTlsCertificateVerifyMessage extends HandshakeMessage {
        ...
        GMTlsCertificateVerifyMessage(HandshakeContext context,
                GMX509Possession gmx509Possession) throws IOException {
            super(context);

            // This happens in client side only.
            ClientHandshakeContext chc = (ClientHandshakeContext)context;
            byte[] temproary = null;
            String algorithm = gmx509Possession.popSignPrivateKey.getAlgorithm();
            try {
                Signature signer =
                        getSignature(algorithm, gmx509Possession.popSignPrivateKey);
                byte[] hashes = chc.handshakeHash.digest();
                signer.update(hashes);
                temproary = signer.sign();
            } catch (NoSuchAlgorithmException nsae) {
                throw chc.conContext.fatal(Alert.INTERNAL_ERROR,
        ...
        GMTlsCertificateVerifyMessage(HandshakeContext context,
            ...
            if (gmx509Credentials == null ||
                    gmx509Credentials.popSignPublicKey == null) {
                throw shc.conContext.fatal(Alert.HANDSHAKE_FAILURE,
                    "No X509 credentials negotiated for CertificateVerify");
            }
            ...
            try {
                Signature signer =
                        getSignature(algorithm, gmx509Credentials.popSignPublicKey);
                byte[] hashes = shc.handshakeHash.digest();
                signer.update(hashes);
                if (!signer.verify(signature)) {
                    throw shc.conContext.fatal(Alert.HANDSHAKE_FAILURE,
                        "Invalid CertificateVerify message: invalid signature");
     ...
     */
    private static final
            class GMTlsCertificateVerifyProducer implements HandshakeProducer {
        // Prevent instantiation of this class.
        private GMTlsCertificateVerifyProducer() {
            // blank
        }
        ...
        public byte[] produce(ConnectionContext context,
            ...
            if (gmx509Possession == null ||
                    gmx509Possession.popSignPrivateKey == null) {
                if (SSLLogger.isOn && SSLLogger.isOn("ssl,handshake")) {
                    SSLLogger.fine(
                        "No X.509 credentials negotiated for CertificateVerify");
                }
            ...
            GMTlsCertificateVerifyMessage cvm =
                    new GMTlsCertificateVerifyMessage(chc, gmx509Possession);
            if (SSLLogger.isOn && SSLLogger.isOn("ssl,handshake")) {
                SSLLogger.fine(
                        "Produced CertificateVerify handshake message", cvm);
            }
     ...
     */
    private static final
    class GMTlsCertificateVerifyConsumer implements SSLConsumer {
        // Prevent instantiation of this class.
        private GMTlsCertificateVerifyConsumer() {
            // blank
        }
        ...
        public void consume(ConnectionContext context,
            ...
            GMTlsCertificateVerifyMessage cvm =
                new GMTlsCertificateVerifyMessage(shc, message);
            if (SSLLogger.isOn && SSLLogger.isOn("ssl,handshake")) {
                SSLLogger.fine(
                        "Consuming CertificateVerify handshake message", cvm);
            }
