 ...
 */
package org.openeuler.tls.sun.security.KeyStore;
...
public class PBETest extends BaseTest {
    private static final String OID_PKCS5_PBES2 = "1.2.840.113549.1.5.13";
//    private final static String DIR = TestUtils.getPath();//System.getProperty("test.src", ".");
    private final static String KEY_PROTECTION_PROPERTY =
        "keystore.PKCS12.keyProtectionAlgorithm";

    private static final String[] PBE_DESede_ALGOS = {
            "PBEWithSHA1AndDESede"
    };

    private static final String[] PBE_AES_ALGOS = {
    ...
    private static final String[] PBE_SM4_ALGOS = {
    ...
    private static final String KEYSTORE = TestUtils.getPath("server.keystore");
    private static final String NEW_KEYSTORE_TYPE = "PKCS12";
    private static final String KEY_ALIAS_RSA = "server-rsa";
    private static final String KEY_ALIAS_SM2 = "server-sm2-enc";
    ...
    public void testDESede() throws Exception {
        test(PBE_DESede_ALGOS, KEY_ALIAS_RSA);
    }

    @Test
    public void testAES()  throws Exception{
        test(PBE_AES_ALGOS, KEY_ALIAS_RSA);
    }

    @Test
    public void testSM4()  throws Exception {
        test(PBE_SM4_ALGOS, KEY_ALIAS_SM2);
    }

    private void test(String[] pbeAlgos, String keyAlias) throws Exception {
        for (String algo : pbeAlgos) {
            String filename = algo.replace("/","-") + ".p12";
            main0(algo, filename, keyAlias, true);
            main0(algo, filename, keyAlias, false);
            Security.setProperty(KEY_PROTECTION_PROPERTY, algo);
            main0(null, "PBE.p12", keyAlias, false);
            Security.setProperty(KEY_PROTECTION_PROPERTY, "");
        }
        main0(null, "default.p12", keyAlias, false); // default algorithm
    }

    private static void main0(String algo, String filename, String keyAlias, boolean useParams)
        throws Exception {
        ...
        System.out.println("Retrieved key entry named '" + keyAlias + "'");
        ...
        if (useParams) {
            ...
            keystore2.setEntry(keyAlias, entry,
                new KeyStore.PasswordProtection(PASSWORD, algo,
                    new PBEParameterSpec(SALT, ITERATION_COUNT, ivParameterSpec)));
        ...
        try {
            ...
            entry = keystore2.getEntry(keyAlias,
                new KeyStore.PasswordProtection(PASSWORD));
            Key key;
            if (entry instanceof KeyStore.PrivateKeyEntry) {
