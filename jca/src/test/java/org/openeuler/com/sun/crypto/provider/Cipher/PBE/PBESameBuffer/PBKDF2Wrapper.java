 ...
 * questions.
 */
package org.openeuler.com.sun.crypto.provider.Cipher.PBE.PBESameBuffer;
...
public class PBKDF2Wrapper extends PBEWrapper {
    ...
    private static final String SM4_CIPHER_TRANSFORMATION = "SM4/CBC/PKCS5Padding";
    ...
    public PBKDF2Wrapper(Provider p, String algo, String passwd,
        ...
        super(algo,
                ...
                Cipher.getInstance(getCipherTransformation(algo), p), out);
        this.keyAlgorithm = getKeyAlgorithm(algo);
    }

    private static String getKeyAlgorithm(String algo) {
        if (algo.toUpperCase().contains("SM3")) {
            return SM4;
        } else {
            return AES;
        }
    ...
    private static String getCipherTransformation(String algo) {
        if (algo.toUpperCase().contains("SM3")) {
            return SM4_CIPHER_TRANSFORMATION;
        } else {
            return AES_CIPHER_TRANSFORMATION;
        }
    }
        ...
        try {
            ...
            if (Cipher.ENCRYPT_MODE == edMode) {
                ...
                ci.init(Cipher.DECRYPT_MODE,
                        new SecretKeySpec(key.getEncoded(), keyAlgorithm),
