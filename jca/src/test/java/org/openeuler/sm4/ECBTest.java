...
public class ECBTest {
    ...
    public void testECB() throws Exception {
        ...
        Cipher bcNopaddingCipher = Cipher.getInstance("SM4/ECB/NOPADDING", "BC");
        Cipher bgmNopaddingCipher = Cipher.getInstance("SM4/ECB/NOPADDING", "BGMJCEProvider");
        Cipher bcWithPaddingCipher = Cipher.getInstance("SM4/ECB/PKCS5Padding", "BC");
        Cipher bgmWithpaddingCipher = Cipher.getInstance("SM4/ECB/PKCS5Padding", "BGMJCEProvider");

        testUpdateAndDofinal(bcWithPaddingCipher, bgmWithpaddingCipher, key, true);
        testUpdateAndDofinal(bcNopaddingCipher, bgmNopaddingCipher, key, false);
        testNoPadding(bcNopaddingCipher, bgmNopaddingCipher, key);
        testWithPadding(bcWithPaddingCipher, bgmWithpaddingCipher, key);
        testUpdate(bcNopaddingCipher, bgmNopaddingCipher, key, false);
        testUpdate(bcWithPaddingCipher, bgmWithpaddingCipher, key, true);
    }

    /**
     * test ECB nopadding mode
     *
     * @param bc
     * @param bgm
     * @param key
     ...
     * @throws Exception
     */
    private void testUpdate(Cipher bc, Cipher bgm, SecretKey key, boolean padding) throws Exception {
        int times = 1000;
        for (int i = 0; i < times; i++) {
            //generate plainText
            int plainTextLen;
            ...
            SecureRandom random = new SecureRandom();
            random.nextBytes(noPaddingPlainText);
            //test update without output args
            bc.init(Cipher.ENCRYPT_MODE, key);
            bgm.init(Cipher.ENCRYPT_MODE, key);
            byte[] clone = noPaddingPlainText.clone();
     ...
     * @throws Exception
     */
    private void testUpdateAndDofinal(Cipher bc, Cipher bgm, SecretKey key, boolean padding) throws Exception {
        int times = 1000;
        for (int i = 0; i < times; i++) {
            //generate plainText
            int plainTextLen;
