...
public class CTRTest {
    ...
    public void testCTR() throws Exception {
        ...
        Cipher bcNopaddingCipher = Cipher.getInstance("SM4/CTR/NOPADDING", "BC");
        Cipher bgmNopaddingCipher = Cipher.getInstance("SM4/CTR/NOPADDING", "BGMJCEProvider");
        Cipher bcWithPaddingCipher = Cipher.getInstance("SM4/CTR/PKCS5Padding", "BC");
        Cipher bgmWithpaddingCipher = Cipher.getInstance("SM4/CTR/PKCS5Padding", "BGMJCEProvider");
    ...
    private void testNoPadding(Cipher bc, Cipher bgm, SecretKey key) throws Exception {
        ...
        for (int i = 0; i < times; i++) {
            ...
            int ivLen = ((int) (Math.random() * 9) + 8);
            while (ivLen == 15) {
                ivLen = ((int) (Math.random() * 9) + 8);
            }
    ...
    private void testUpdateAndDofinal(Cipher bc, Cipher bgm, SecretKey key) throws Exception {
        ...
        for (int i = 0; i < times; i++) {
            ...
            //combine combine all encrypted results
            byte[] bcArr = null;
            byte[] bgmArr = null;
