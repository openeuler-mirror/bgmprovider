...
public class GCMTest {
    ...
    public void testGCM() throws Exception {
        ...
        Cipher bcNopaddingCipher = Cipher.getInstance("SM4/GCM/NOPADDING", "BC");
        Cipher bgmNopaddingCipher = Cipher.getInstance("SM4/GCM/NOPADDING", "BGMJCEProvider");
    ...
    private int[] tLenArr = new int[]{128, 120, 112, 104, 96};

    /**
     * Encryption and decryption tests
     * for GCM only no padding is supported
     *
     * @param bc
     * @param bgm
     * @param key
    ...
    private void testNoPadding(Cipher bc, Cipher bgm, SecretKey key) throws Exception {
        ...
        for (int i = 0; i < times; i++) {
            ...
            int ivLen = (int) (Math.random() * 16) + 1;
            byte[] iv = new byte[ivLen];
            new SecureRandom().nextBytes(iv);
            int tLenArrIndex = (int) (Math.random() * 5);
            ...
            //regenerate iv and authentication tag length
            //cannot reuse nonce for GCM encryption
            ivLen = (int) (Math.random() * 16) + 1;
            iv = new byte[ivLen];
            new SecureRandom().nextBytes(iv);
            tLenArrIndex = (int) (Math.random() * 5);
            gcmParam = new GCMParameterSpec(tLenArr[tLenArrIndex], iv);
