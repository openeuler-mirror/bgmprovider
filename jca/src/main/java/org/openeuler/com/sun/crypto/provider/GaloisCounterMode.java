...
final class GaloisCounterMode extends FeedbackCipher {
    ...
    static int DEFAULT_TAG_LEN = SM4_BLOCK_SIZE;
    ...
    static void increment32(byte[] value) {
        if (value.length != SM4_BLOCK_SIZE) {
            // should never happen
    ...
    private static byte[] getLengthBlock(int ivLenInBytes) {
        ...
        byte[] out = new byte[SM4_BLOCK_SIZE];
        out[8] = (byte)(ivLen >>> 56);
        out[9] = (byte)(ivLen >>> 48);
    ...
    private static byte[] expandToOneBlock(byte[] in, int inOfs, int len) {
        ...
        if (len == SM4_BLOCK_SIZE && inOfs == 0) {
            return in;
        } else {
            byte[] paddedIn = new byte[SM4_BLOCK_SIZE];
    ...
    private static byte[] getJ0(byte[] iv, byte[] subkeyH) {
        ...
        if (iv.length == 12) { // 96 bits
            ...
            j0[SM4_BLOCK_SIZE - 1] = 1;
        } else {
            GHASH g = new GHASH(subkeyH);
            int lastLen = iv.length % SM4_BLOCK_SIZE;
            if (lastLen != 0) {
    ...
    void init(boolean decrypting, String algorithm, byte[] keyValue,
        ...
        this.subkeyH = new byte[SM4_BLOCK_SIZE];
        this.embeddedCipher.encryptBlock(new byte[SM4_BLOCK_SIZE], 0,
                this.subkeyH, 0);
    ...
    void processAAD() {
        if (aadBuffer != null) {
            if (aadBuffer.size() > 0) {
                ...
                int lastLen = aad.length % SM4_BLOCK_SIZE;
                if (lastLen != 0) {
    ...
    void doLastBlock(byte[] in, int inOfs, int len, byte[] out, int outOfs,
        ...
        if (len > TRIGGERLEN) {
            ...
            final int plen = SM4_BLOCK_SIZE * 6;
        ...
        int lastLen = ilen % SM4_BLOCK_SIZE;
        if (lastLen != 0) {
