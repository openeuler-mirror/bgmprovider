...
final class GCTR extends CounterMode {
    ...
    GCTR(SymmetricCipher cipher, byte[] initialCounterBlk) {
        super(cipher);
        if (initialCounterBlk.length != SM4_BLOCK_SIZE) {
            throw new RuntimeException("length of initial counter block (" + initialCounterBlk.length +
                                       ") not equal to AES_BLOCK_SIZE (" + SM4_BLOCK_SIZE + ")");
        }
    ...
    int update(byte[] in, int inOfs, int inLen, byte[] out, int outOfs) {
        ...
        int numOfCompleteBlocks = inLen / SM4_BLOCK_SIZE;
        if (numOfCompleteBlocks >= blocksLeft) {
            ...
            byte[] encryptedCntr = new byte[SM4_BLOCK_SIZE];
            for (int i = 0; i < numOfCompleteBlocks; i++) {
                embeddedCipher.encryptBlock(counter, 0, encryptedCntr, 0);
                for (int n = 0; n < SM4_BLOCK_SIZE; n++) {
                    int index = (i * SM4_BLOCK_SIZE + n);
                    out[outOfs + index] =
    ...
    int doFinal(byte[] in, int inOfs, int inLen, byte[] out,
        ...
        try {
            if (inLen < 0) {
                ...
                int lastBlockSize = inLen % SM4_BLOCK_SIZE;
