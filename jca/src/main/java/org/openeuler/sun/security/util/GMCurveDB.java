 ...
 * This class is a modified version of the CurveDB class in the Sun library, with the main difference being
 * the addition of two sets of standard elliptic curve parameters for the SM2 algorithm.
 * @see sun.security.util.CurveDB
 */
public class  GMCurveDB {
    ...
    // Return a ECNamedCurve for the specified OID/name or null if unknown.
    static ECNamedCurve lookup(String name) {
        ECNamedCurve spec = oidMap.get(name);
        if (spec != null) {
            return spec;
        }
    ...
    // Convert the given ECParameterSpec object to a ECNamedCurve object.
    // If params does not represent a known named curve, return null.
    static ECNamedCurve lookup(ECParameterSpec params) {
        if ((params instanceof ECNamedCurve) || (params == null)) {
            return (ECNamedCurve)params;
        }
        ...
        // curves using the short form, not explicitly.
        int fieldSize = params.getCurve().getField().getFieldSize();
        for (ECNamedCurve namedCurve : specCollection) {
    ...
    static String[] getNamesByOID(String oid) {
        ECNamedCurve nc = oidMap.get(oid);
        if (nc == null) {
            return new String[0];
        }
